...
document.addEventListener("DOMContentLoaded", function () {
  // localStorage.getItem("cartItems") để lấy dữ liệu lưu trong localstorage
  const cartItemsStr = localStorage.getItem("cartItems")
  // vì lưu vào là JSON string, nên phải parse nó về lại JSON array để dùng tiếp
  const cartItems = cartItemsStr ? JSON.parse(cartItemsStr) : []
  // Chọn thẻ <span> trong class "icon-cart"
  const iconCartSpan = document.querySelector(".icon-cart span")
  // lấy hàm này ra để load các sản phẩm đang lưu
  updateCartUI();
  ...
  function addToCart(productName, price) {
    ...
    // sau khi cập nhật, thêm, xóa item thì phải cập nhật lại localStorage luôn
    // giá trị lưu vào localStorage không được lưu object hay array object, phải parse nó về JSON string
  ...
  //  // Gọi hàm để cập nhật số lượng từ localStorage
  ...
  addToCartButtons.forEach((button) => {
    button.addEventListener("click", function () {
      ...
      const productName =
        productContainer.querySelector(".ProductName").textContent
      const priceString = productContainer
        .querySelector(".Price")
        .textContent.replace("$", "")
        .replace(",", "")
